...
$sql = ("SELECT * FROM `pedidos_proveedor` WHERE id_sucursal = $id_sucursal");
                    ...
                    <table class="table table-striped table-bordered" id="productosTable">
                        ...
                        <tbody>
                            <?php while ($producto = mysqli_fetch_assoc($resultado)) { ?>
                                <tr>
                                    <td><?= htmlspecialchars($producto['id_pedido_proveedor']) ?></td>
                                    <td><?= htmlspecialchars($producto['id_proveedor']) ?></td>
                                    <td><?= htmlspecialchars($producto['producto']) ?></td>
                                    <td><?= htmlspecialchars($producto['cantidad']) ?></td>
                                    <td><?= htmlspecialchars($producto['metodo_pago']) ?></td>
                                    ...
                                    <td class='fecha-pedido'><?= htmlspecialchars($producto['fecha_pedido']) ?></td>

                                    <!-- Acciones: Condicionar la visualización de botones -->
                                    <td class='acciones'>
                                        <?php if ($producto['estado_pedido'] == 'pendiente'): ?>
                                            <!-- Botón para cancelar el pedido -->
                                            <button class='btn btn-danger cancelar-pedido' data-id='<?= htmlspecialchars($producto['id_pedido_proveedor']) ?>' data-fecha-pedido='<?= htmlspecialchars($producto['fecha_pedido']) ?>'>
                                                Cancelar pedido
                                            </button>

                                            <!-- Botón para marcar la llegada del pedido -->
                                            <button class='btn btn-success llego-pedido' data-id='<?= htmlspecialchars($producto['id_pedido_proveedor']) ?>' data-fecha-pedido='<?= htmlspecialchars($producto['fecha_pedido']) ?>'>
                                                Llegó pedido
                                            </button>
        ...
        function confirmarAccion(accion, fechaPedido) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = `${urlAccion}?fecha_pedido=${fechaPedido}`;
                }
            });
        }
    </script>
